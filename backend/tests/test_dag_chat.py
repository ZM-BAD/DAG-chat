...
def build_dag_from_parents(mongo_db: MockMongoDB, parent_ids: list[str]) -> tuple[dict, dict]:
    """
...
class TestComplexDAG:
    ...
    def complex_dag_db(self):
        ...
        # a: 根节点，无parent_ids
        db.insert_node(MockMessageNode(
            id='user_a', role='user', content=USER_QUESTIONS['a'], parent_ids=[]
        ))
        db.insert_node(MockMessageNode(
            id='assistant_a', role='assistant', content=ASSISTANT_ANSWERS['a'], 
            parent_ids=['user_a'], children=['user_b', 'user_c', 'user_d', 'user_e']
        ))
        
        # b-f: 北京分支
        db.insert_node(MockMessageNode(
            id='user_b', role='user', content=USER_QUESTIONS['b'], parent_ids=['assistant_a']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_b', role='assistant', content=ASSISTANT_ANSWERS['b'],
            parent_ids=['user_b'], children=['user_f', 'user_g']
        ))
        
        db.insert_node(MockMessageNode(
            id='user_f', role='user', content=USER_QUESTIONS['f'], parent_ids=['assistant_b']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_f', role='assistant', content=ASSISTANT_ANSWERS['f'], parent_ids=['user_f']
        ))
        
        # g: 北京旅游
        db.insert_node(MockMessageNode(
            id='user_g', role='user', content=USER_QUESTIONS['g'], parent_ids=['assistant_b']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_g', role='assistant', content=ASSISTANT_ANSWERS['g'], parent_ids=['user_g']
        ))
        
        # c-h, i: 上海分支
        db.insert_node(MockMessageNode(
            id='user_c', role='user', content=USER_QUESTIONS['c'], parent_ids=['assistant_a']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_c', role='assistant', content=ASSISTANT_ANSWERS['c'],
            parent_ids=['user_c'], children=['user_h', 'user_i']
        ))
        
        db.insert_node(MockMessageNode(
            id='user_h', role='user', content=USER_QUESTIONS['h'], parent_ids=['assistant_c']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_h', role='assistant', content=ASSISTANT_ANSWERS['h'], parent_ids=['user_h']
        ))
        
        db.insert_node(MockMessageNode(
            id='user_i', role='user', content=USER_QUESTIONS['i'], parent_ids=['assistant_c']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_i', role='assistant', content=ASSISTANT_ANSWERS['i'],
            parent_ids=['user_i'], children=['user_n']
        ))
        
        # d-j, k: 广州分支
        db.insert_node(MockMessageNode(
            id='user_d', role='user', content=USER_QUESTIONS['d'], parent_ids=['assistant_a']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_d', role='assistant', content=ASSISTANT_ANSWERS['d'],
            parent_ids=['user_d'], children=['user_j', 'user_k']
        ))
        
        db.insert_node(MockMessageNode(
            id='user_j', role='user', content=USER_QUESTIONS['j'], parent_ids=['assistant_d']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_j', role='assistant', content=ASSISTANT_ANSWERS['j'],
            parent_ids=['user_j'], children=['user_n', 'user_o', 'user_s']
        ))
        
        db.insert_node(MockMessageNode(
            id='user_k', role='user', content=USER_QUESTIONS['k'], parent_ids=['assistant_d']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_k', role='assistant', content=ASSISTANT_ANSWERS['k'],
            parent_ids=['user_k'], children=['user_p', 'user_t']
        ))
        
        # e-l, m: 深圳分支
        db.insert_node(MockMessageNode(
            id='user_e', role='user', content=USER_QUESTIONS['e'], parent_ids=['assistant_a']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_e', role='assistant', content=ASSISTANT_ANSWERS['e'],
            parent_ids=['user_e'], children=['user_l', 'user_m']
        ))
        
        db.insert_node(MockMessageNode(
            id='user_l', role='user', content=USER_QUESTIONS['l'], parent_ids=['assistant_e']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_l', role='assistant', content=ASSISTANT_ANSWERS['l'], parent_ids=['user_l']
        ))
        
        db.insert_node(MockMessageNode(
            id='user_m', role='user', content=USER_QUESTIONS['m'], parent_ids=['assistant_e']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_m', role='assistant', content=ASSISTANT_ANSWERS['m'], parent_ids=['user_m']
        ))
        
        # n: 合并节点（来自i和j）
        db.insert_node(MockMessageNode(
            id='user_n', role='user', content=USER_QUESTIONS['n'], parent_ids=['assistant_i', 'assistant_j']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_n', role='assistant', content=ASSISTANT_ANSWERS['n'],
            parent_ids=['user_n'], children=['user_s']
        ))
        
        # o-q-s链
        db.insert_node(MockMessageNode(
            id='user_o', role='user', content=USER_QUESTIONS['o'], parent_ids=['assistant_j']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_o', role='assistant', content=ASSISTANT_ANSWERS['o'],
            parent_ids=['user_o'], children=['user_q']
        ))
        
        db.insert_node(MockMessageNode(
            id='user_q', role='user', content=USER_QUESTIONS['q'], parent_ids=['assistant_o']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_q', role='assistant', content=ASSISTANT_ANSWERS['q'],
            parent_ids=['user_q'], children=['user_s', 'user_t']
        ))
        
        # p-r链
        db.insert_node(MockMessageNode(
            id='user_p', role='user', content=USER_QUESTIONS['p'], parent_ids=['assistant_k']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_p', role='assistant', content=ASSISTANT_ANSWERS['p'],
            parent_ids=['user_p'], children=['user_r']
        ))
        
        db.insert_node(MockMessageNode(
            id='user_r', role='user', content=USER_QUESTIONS['r'], parent_ids=['assistant_p']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_r', role='assistant', content=ASSISTANT_ANSWERS['r'],
            parent_ids=['user_r'], children=['user_t']
        ))
        
        # s: 合并节点（来自n、j、q）
        db.insert_node(MockMessageNode(
            id='user_s', role='user', content=USER_QUESTIONS['s'], 
            parent_ids=['assistant_n', 'assistant_j', 'assistant_q']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_s', role='assistant', content=ASSISTANT_ANSWERS['s'], parent_ids=['user_s']
        ))
        
        # t: 合并节点（来自k、q、r）
        db.insert_node(MockMessageNode(
            id='user_t', role='user', content=USER_QUESTIONS['t'], 
            parent_ids=['assistant_k', 'assistant_q', 'assistant_r']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_t', role='assistant', content=ASSISTANT_ANSWERS['t'], parent_ids=['user_t']
        ))
    ...
    def test_subdag_building_for_merge_node(self, complex_dag_db):
        ...
        expected_nodes = {
            'user_a', 'assistant_a',
            'user_c', 'assistant_c', 'user_h', 'assistant_h',
            'user_d', 'assistant_d', 'user_j', 'assistant_j',
            'user_i', 'assistant_i', 'user_n', 'assistant_n',
            'user_o', 'assistant_o', 'user_q', 'assistant_q',
            'user_s', 'assistant_s'
        }
        
        actual_nodes = set(node_map.keys())
        assert expected_nodes <= actual_nodes, f"缺少节点: {expected_nodes - actual_nodes}"
    ...
    def test_topological_sort_for_merge_node(self, complex_dag_db):
        ...
        def assert_before(parent, child, msg=""):
            ...
            if parent_indices and child_indices:
                assert max(parent_indices) < min(child_indices), msg or f"{parent}必须在{child}之前"
...
class TestLinkedListScenario:
    ...
    def linked_list_db(self):
        ...
        db.insert_node(MockMessageNode(
            id='assistant_a', role='assistant', content=ASSISTANT_ANSWERS['a'],
            parent_ids=['user_a'], children=['user_b']
        ))
        ...
        db.insert_node(MockMessageNode(
            id='assistant_b', role='assistant', content=ASSISTANT_ANSWERS['b'],
            parent_ids=['user_b'], children=['user_c']
        ))
        
        # c
        db.insert_node(MockMessageNode(
            id='user_c', role='user', content=USER_QUESTIONS['c'], parent_ids=['assistant_b']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_c', role='assistant', content=ASSISTANT_ANSWERS['c'],
            parent_ids=['user_c'], children=['user_d']
        ))
        
        # d
        db.insert_node(MockMessageNode(
            id='user_d', role='user', content=USER_QUESTIONS['d'], parent_ids=['assistant_c']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_d', role='assistant', content=ASSISTANT_ANSWERS['d'],
            parent_ids=['user_d'], children=['user_e']
        ))
        
        # e
        db.insert_node(MockMessageNode(
            id='user_e', role='user', content=USER_QUESTIONS['e'], parent_ids=['assistant_d']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_e', role='assistant', content=ASSISTANT_ANSWERS['e'], parent_ids=['user_e']
        ))
    ...
    def test_linked_list_topological_sort(self, linked_list_db):
        ...
        expected_order = ['user_a', 'assistant_a', 'user_b', 'assistant_b', 
                         'user_c', 'assistant_c', 'user_d', 'assistant_d',
                         'user_e', 'assistant_e']
        
        assert sorted_nodes == expected_order, f"拓扑排序应保持一致性\n实际: {sorted_nodes}\n预期: {expected_order}"
...
class TestTreeScenario:
    ...
    def tree_db(self):
        ...
        db.insert_node(MockMessageNode(
            id='assistant_a', role='assistant', content=ASSISTANT_ANSWERS['a'],
            parent_ids=['user_a'], children=['user_b', 'user_c', 'user_d']
        ))
        ...
        db.insert_node(MockMessageNode(
            id='assistant_b', role='assistant', content=ASSISTANT_ANSWERS['b'],
            parent_ids=['user_b'], children=['user_e', 'user_f']
        ))
        ...
        db.insert_node(MockMessageNode(
            id='assistant_c', role='assistant', content=ASSISTANT_ANSWERS['c'], parent_ids=['user_c']
        ))
        ...
        db.insert_node(MockMessageNode(
            id='assistant_d', role='assistant', content=ASSISTANT_ANSWERS['d'],
            parent_ids=['user_d'], children=['user_g', 'user_h']
        ))
        
        # e, f (b的子节点)
        db.insert_node(MockMessageNode(
            id='user_e', role='user', content=USER_QUESTIONS['e'], parent_ids=['assistant_b']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_e', role='assistant', content=ASSISTANT_ANSWERS['e'], parent_ids=['user_e']
        ))
        ...
        # g, h (d的子节点)
        db.insert_node(MockMessageNode(
            id='user_g', role='user', content=USER_QUESTIONS['g'], parent_ids=['assistant_d']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_g', role='assistant', content=ASSISTANT_ANSWERS['g'], parent_ids=['user_g']
        ))
        ...
        db.insert_node(MockMessageNode(
            id='user_h', role='user', content=USER_QUESTIONS['h'], parent_ids=['assistant_d']
        ))
        db.insert_node(MockMessageNode(
            id='assistant_h', role='assistant', content=ASSISTANT_ANSWERS['h'], parent_ids=['user_h']
        ))
        ...
        for node_id, node in db._nodes.items():
            if node.role == 'user':
                if node_id == 'user_a':
                    ...
                    assert len(node.parent_ids) == 1, f"树中{node_id}应该有且只有一个parent_id"
            else:  # assistant
                assert len(node.parent_ids) == 1, f"{node_id}应该有且只有一个parent_id"
    ...
    def test_tree_no_merge_points(self, tree_db):
        ...
        for node_id, node in db._nodes.items():
            assert len(node.parent_ids) <= 1, f"树中不应该有合并点，但{node_id}有{len(node.parent_ids)}个parent"
    ...
    def test_tree_topological_sort_from_leaf(self, tree_db):
        ...
        # 验证包含的节点: a, b, f
        expected_nodes = {'user_a', 'assistant_a', 'user_b', 'assistant_b', 'user_f', 'assistant_f'}
        actual_nodes = set(node_map.keys())
    ...
    def test_tree_subdag_from_multiple_leaves(self, tree_db):
        ...
        expected_nodes = {
            'user_a', 'assistant_a',
            'user_b', 'assistant_b', 'user_e', 'assistant_e',
            'user_d', 'assistant_d', 'user_h', 'assistant_h'
        }
        actual_nodes = set(node_map.keys())
...
class TestEdgeCases:
    ...
    def test_single_node(self):
        ...
        db = MockMongoDB()
        db.insert_node(MockMessageNode(
            id='user_a', role='user', content='test', parent_ids=[]
        ))
        db.insert_node(MockMessageNode(
            id='assistant_a', role='assistant', content='response',
            parent_ids=['user_a']
        ))
...
def test_complex_dag_with_user_questions():
    ...
    relationships = [
        ...
        ('k', 'p'),
        ('o', 'q'),
        ('j', 's'), ('n', 's'), ('q', 's'),  # s有多个父节点（合并点）
        ('p', 'r'),
        ('k', 't'), ('q', 't'), ('r', 't'),  # t有多个父节点（合并点）
    ]
    ...
    # 先创建u节点
    db.insert_node(MockMessageNode(
        id='user_u', role='user', content=USER_QUESTIONS['u'],
        parent_ids=['assistant_h', 'assistant_s']
    ))
    db.insert_node(MockMessageNode(
        id='assistant_u', role='assistant', content=ASSISTANT_ANSWERS['u'],
        parent_ids=['user_u']
    ))
    ...
    def assert_before(parent, child):
        ...
        assert parent_idx < child_idx, f"{parent}({parent_idx})必须在{child}({child_idx})之前"
