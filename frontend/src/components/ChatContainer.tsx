...
const ChatContainer: React.FC<ChatContainerProps> = ({
  ...
  // 状态管理parent分支选择
  const [selectedParents, setSelectedParents] = useState<Map<string, string>>(new Map());

  // 构建对话树和用户消息分组、parent分组
  const { userMessageGroups, parentMessageGroups, displayMessages } = useMemo(() => {
    ...
    const displayMsgs = getCompleteConversationPath(tree, mergedSelectedBranches, mergedSelectedParents);
  ...
  useEffect(() => {
    ...
    recentMessages.forEach(message => {
      ...
      if (message.role === 'user' && message.parent_ids && message.parent_ids.length > 1) {
  ...
  // 处理parent分支选择
  const handleParentSelect = useCallback((userMessageId: string, selectedParentId: string) => {
      ...
      {shouldShowWelcome ? (
        ...
        <div className="chat-messages">
          {displayMessages.map((message, index) => {
            ...
            const isParentOfMultiParentUser = message.role === 'assistant' && 
              Array.from(parentMessageGroups.values()).some(parents => 
                parents.some(p => p.id === message.id)
              );

            const hasChildrenBranch = message.role === 'user' && 
              userMessageGroups.has(message.id) && 
              userMessageGroups.get(message.id)!.length > 1;
            ...
            if (message.role === 'user' && index > 0) {
              ...
              if (prevMsg.role === 'assistant' && userMessageGroups.has(prevMsg.id)) {
            ...
            if (message.role === 'assistant' && isParentOfMultiParentUser && parentUserId) {
              return (
                <div key={`merge-${message.id}`} className="merge-unit">
                  ...
                  <ConversationBranchTabs
                    ...
                    selectedBranchId={selectedParents.get(parentUserId) || parentMessageGroups.get(parentUserId)![0].id}
                    onBranchSelect={(parentId) => handleParentSelect(parentUserId!, parentId)}
                    iconType="merge"
                  />
                </div>
              );
              ...
              return (
                <div key={`branch-${message.id}`} className="branch-unit">
                  <ConversationBranchTabs
                    ...
                    selectedBranchId={selectedBranches.get(prevAssistant.id) || userMessageGroups.get(prevAssistant.id)![0].id}
                    onBranchSelect={(branchId) => handleBranchSelect(prevAssistant.id, branchId)}
                    iconType="branch"
                  />
                  <ChatMessage
                    message={message}
